      ...
      <div className="max-w-6xl mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-6">
            {navLinks.map((link) => (
              <Link key={link.href} href={link.href} className="text-sm text-white/70 hover:text-[#f5a623] transition-colors">
                {link.label}
              </Link>
            ))}
          </div>
        ...
        {mobileMenuOpen && (
          <div className="md:hidden bg-[#1a1a2e] border-t border-white/10 py-4">
            {navLinks.map((link) => (
              <Link key={link.href} href={link.href} className="block px-4 py-3 text-sm text-white/70 hover:text-[#f5a623] hover:bg-white/5 transition-colors" onClick={() => setMobileMenuOpen(false)}>
                {link.label}
              </Link>
            ))}
            <div className="px-4 pt-2">
              <Link href="/demo" className="block text-center px-4 py-2 rounded-lg bg-[#e94560] text-white text-sm" onClick={() => setMobileMenuOpen(false)}>
                Book Demo
              </Link>
            </div>
          </div>
